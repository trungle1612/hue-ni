...
React Router v7. Five routes:
- `/` — HomePage: cream header + full-screen Leaflet map + floating filter chips + 3-snap-point place bottom sheet (`PlaceDetailSheet`)
- `/festivals` — FestivalPage: tabbed festival/event browser (replaced `/heritage`)
- `/my-trip` — MyTripPage: saved places from localStorage
- `/details/:id` — DetailsPage: renders **outside** `AppLayout` (no bottom nav)
- `/festivals/:id` — FestivalDetailPage: renders **outside** `AppLayout` (no bottom nav)
...
`Place` fields: `id`, `name`, `category`, `tags`, `coverImage`, `gallery`, `address`, `coordinates` (`{lat, lng}`), `rating`, `priceRange` (`₫/₫₫/₫₫₫`), `hours` (`{open, close}`), `vibe`, `description`, `insiderTips[]`; optional: `specialty`, `collection`, `phone`, `website`, `logo`, `reviews` (`Review[]`), `menu` (array of image URLs — cafes/food only), `foodTags` (`FoodTags` with `dishType[]`, `mealTime[]`, `vibe[]` — food category only). Collections: `Collection[]` (`{id, title, description}`).

`festivals.json` has shape `{ events: FestivalEvent[], stories: StoryAndGuide[] }`. `FestivalEvent` fields: `id`, `title`, `category` (`EventCategory`), `isAnnual`, `summary`, `highlights[]`, `timeString`, `location`, `organizer`, `date` (`{start, end?}`); optional: `season`, `month`, `thumbnail`, `linkedStoryId`. `StoryAndGuide` fields: `id`, `type` (`royal_anecdote | survival_guide`), `title`, `excerpt`, `content`. Festival utilities in `src/utils/festivalUtils.ts` (`getEventBadge`, `sortEventsByDate`, `filterEventsByCategory`).
...
Saved-places list lives in `src/hooks/useMyTrip.ts` — reads/writes `localStorage` key `hue-ni-trip` (JSON array of place IDs). The hook is instantiated **once** inside `MyTripProvider` (`src/contexts/MyTripContext.tsx`), which wraps the entire app in `App.tsx`. All consumers (`BottomNav`, `PlaceCard`, `PlaceDetailSheet`, `MyTripPage`, `DetailsPage`) call `useMyTripContext()` — never `useMyTrip()` directly — so bookmark state is shared and updates propagate everywhere (e.g. the BottomNav badge count on `/my-trip`, the gold dot on saved map pins, the bookmark button state in the bottom sheet).

`HomePage` passes `savedIds` down to `MapView` as a prop (not via context) since MapView is a generic component that doesn't own global state. `HomePage` owns `snapState: SnapState` (`'closed' | 'peek' | 'mid' | 'full'`) and `selectedPlace: Place | null` — `snapState` drives which sheet level is shown, `selectedPlace` drives the map pin highlight. When `onClose` fires from the sheet, `snapState` goes to `'closed'` immediately and `selectedPlace` clears after 300ms (matching the CSS exit animation).
...
Two variants: `vertical` (full-width list, 2-line name clamp) and `horizontal` (Home carousel, 200px, single-line truncated name, vibe hidden).
...
### FestivalPage (`src/pages/FestivalPage/`)

Three tabs: `timeline` (Sự kiện), `calendar` (Lịch), `culture` (Văn hóa). Lazy-loads `festivals.json` on mount. Owns `selectedEvent: FestivalEvent | null` and `sheetOpen: boolean` — same deferred-clear pattern as `HomePage` (300ms timeout matching CSS exit animation). Uses `FestivalEventSheet` for event details (mirrors `PlaceDetailSheet` but for events). `FestivalDetailPage` renders outside `AppLayout` for direct URL access.

### PlaceDetailSheet (`src/components/PlaceDetailSheet/index.tsx`)

Google Maps-style 3-snap-point bottom sheet used on **HomePage only** (MyTrip still navigates to `/details/:id`). Exports `SnapState` type.

Three snap states driven by the parent via `snapState` prop:
- **peek** (`148px`) — thumbnail, name, rating, address, "Xem chi tiết ↑" CTA, save/share icons
- **mid** (`52vh`) — cover image, name, vibe, meta row, address, TagList, directions/save/share buttons
- **full** (`92vh - --bottom-nav-height`) — full info grid, description, gallery, tabs (Mẹo/Đánh giá/Thực đơn), lightbox

Drag gestures on the handle only (`onPointerDown/Up` + `setPointerCapture`). 50px threshold to advance/retreat. Uses `useMyTripContext()` directly for save/unsave; calls `onSave?.()` callback so HomePage can show the toast. Tab/review state resets when `place.id` changes (inline render-phase comparison pattern).

### ReviewCard (`src/components/ReviewCard.tsx`)

Extracted from DetailsPage. Exports `ReviewCard` component and `LightboxState` type (`{ images: string[]; index: number }`). Used by both `DetailsPage` and `PlaceDetailSheet`.

### TagList (`src/components/TagList.tsx`)

Extracted from DetailsPage. Padding-agnostic — callers add spacing via contextual CSS (e.g. `.details-page .tag-list`). Used by both `DetailsPage` and `PlaceDetailSheet`.
...
Three tabs: `tips`, `menu`, `reviews` (defined in `TAB_ORDER` — display order: Mẹo → Thực đơn → Đánh giá). The menu tab is only surfaced for `cafe` and `food` categories — `visibleTabs` filters it out for other categories, and `effectiveTab` falls back to `visibleTabs[0]` if the stored `activeTab` is no longer visible. The sliding indicator uses `--tab-index` CSS var driven by `visibleTabs.findIndex(effectiveTab)`. `PlaceDetailSheet` mirrors this exact pattern with `--tab-count` additionally passed so the indicator width is dynamic for 2 or 3 tabs.
...
src/pages/
  ...
  FestivalPage/        index.tsx + style.css + sub-components (EventCard, FestivalEventSheet, TimelineTab, CalendarTab, CultureTab)
  FestivalDetailPage/  index.tsx + style.css
  MyTripPage/          index.tsx + style.css
  DetailsPage/         index.tsx + style.css
  HeritagePage/        index.tsx + style.css (unused — not routed)
```
