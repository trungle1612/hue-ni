...
path = 'src/data/quan-an-dia-phuong'
...
_NAME_TO_DISH = [
    ...
    (r'bánh ép', 'banh-ep'),
    (r'bánh bèo|bánh nậm|bánh lọc|nậm lọc|nậm-lọc|\bbèo\b.{0,15}\bnậm\b|\bnậm\b.{0,15}\bbèo\b', 'banh-beo'),
...
def _infer_dish_types(name):
    lowered = name.lower()
    ascii_lower = _strip(lowered)
    results = []
    for pattern, dish in _NAME_TO_DISH:
        if (re.search(pattern, lowered) or re.search(_strip(pattern), ascii_lower)) and dish not in results:
            results.append(dish)
    return results
...
def generate_food_tags(name, r):
    ...
    dish_types = _infer_dish_types(name)
    # Always add the source category tag for this import batch
    if 'quan-an-dia-phuong' not in dish_types:
        dish_types.append('quan-an-dia-phuong')
...
def generate_vibe(name, r):
    ...
    if 'nem lụi' in n: return 'Nem lụi nướng than hoa — thơm phức, chấm mắm me đặc trưng xứ Huế.'
    if 'bánh tráng cuốn' in n: return 'Bánh tráng cuốn heo quay — giòn tan, thơm ngon, đặc sản cuốn xứ Huế.'
    if 'mắm nêm' in n or 'mắm huế' in n: return 'Bún mắm nêm thơm nồng — nước chấm sánh mịn, đặc sản không nơi nào có.'
    if 'chay' in n or 'vegetarian' in n: return 'Bếp chay thanh tịnh — ngon lành, nhẹ nhàng và tốt cho sức khoẻ.'
...
for csv_path in csv_files:
    ...
    for r in rows:
        ...
        entry = {
            ...
            'foodTags':    generate_food_tags(name, r),
        }
